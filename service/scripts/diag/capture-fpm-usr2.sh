...
# КОНТЕЙНЕРА (НЕ webroot/bind-mount → nginx недостижим, исчезает при любом
# recreate). 150с > graceful-окна и > request_terminate_timeout=120s —
# проверяет и drain, и SIGKILL-на-120s путь.
HOLD_SECONDS="${HOLD_SECONDS:-0}"
while [ $# -gt 0 ]; do
  case "$1" in
    --no-fire)            FIRE_USR2=0 ;;
    --hold-seconds)       shift; HOLD_SECONDS="${1:-150}" ;;
    --hold-seconds=*)     HOLD_SECONDS="${1#*=}" ;;
    *) echo "WARN: неизвестный аргумент '$1' — игнор" >&2 ;;
  esac
  shift
done
...
# ── Предусловие H1: занять один воркер длинным запросом (опц.) ─────────────
HOLD_TMP=""
hold_cleanup(){ [ -n "$HOLD_TMP" ] && docker exec "$CONTAINER" rm -f "$HOLD_TMP" 2>/dev/null; }
if [ "$HOLD_SECONDS" -gt 0 ] 2>/dev/null; then
  [ "$HAVE_FCGI" -eq 1 ] || { echo "FATAL: --hold-seconds требует cgi-fcgi в контейнере" >&2; exit 1; }
  HOLD_TMP="/tmp/diag-hold-$TS.php"
  trap hold_cleanup EXIT INT TERM
  # Голый sleep, без WP-обвязки. set_time_limit(0): max_execution_time не
  # должен срубить раньше; FPM request_terminate_timeout=120s срубит на 120s
  # (это часть проверяемого пути).
  docker exec "$CONTAINER" sh -c \
    "printf '%s' '<?php set_time_limit(0); sleep($HOLD_SECONDS);' > $HOLD_TMP && chmod 644 $HOLD_TMP" \
    || { echo "FATAL: не создать $HOLD_TMP" >&2; exit 1; }
  # Запускаем запрос в ПУЛ :9000 в фоне (detached) — занимает 1 воркер.
  docker exec -d "$CONTAINER" sh -c \
    "SCRIPT_NAME=/diag-hold SCRIPT_FILENAME=$HOLD_TMP REQUEST_METHOD=GET \
     cgi-fcgi -bind -connect 127.0.0.1:9000 >/dev/null 2>&1"
  log "H1: запущен hold-запрос ${HOLD_SECONDS}s на :9000 ($HOLD_TMP)"
  # Ждём, пока FPM-статус покажет active processes >= 1 (до 15s).
  ok=0
  for _ in $(seq 1 30); do
    a="$(fpm_status 2>/dev/null | tr -d '\n' \
          | grep -oE '"active processes":[0-9]+' | grep -oE '[0-9]+$')"
    if [ -n "$a" ] && [ "$a" -ge 1 ] 2>/dev/null; then ok=1; break; fi
    sleep 0.5
  done
  if [ "$ok" -eq 1 ]; then
    log "H1: подтверждено — active processes>=1 (воркер занят перед USR2)"
  else
    log "WARN: H1 — не вижу active>=1 за 15s; продолжаю, но проверь pre-usr2.txt"
  fi
else
  log "режим без предусловия (HOLD_SECONDS=0) — baseline idle-USR2"
fi
...
EVENTS_PID=$!
cleanup() { kill "$LOGS_PID" "$EVENTS_PID" "${SAMP_PID:-}" 2>/dev/null; hold_cleanup; }
...

{
  ...
  if [ "$HOLD_SECONDS" -gt 0 ] 2>/dev/null; then
    echo "ПРЕДУСЛОВИЕ H1: АКТИВНО — hold-запрос ${HOLD_SECONDS}s занимал 1 воркер на :9000 в момент USR2"
  else
    echo "ПРЕДУСЛОВИЕ H1: нет (baseline idle-USR2)"
  fi
  hr
