...
# Точечное чтение значения из compose-.env без `source`: compose-формат шире
# bash-формата (значения с regex/() — например MODSEC_AUDIT_LOG_RELEVANT_STATUS —
# валят bash-парсер). Возвращает пустую строку, если файл/ключ отсутствует.
read_env() {
